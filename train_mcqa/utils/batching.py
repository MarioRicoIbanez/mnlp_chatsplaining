...
class SmartPaddingTokenBatchSampler(Sampler):
    ...
    def __init__(
        ...
        if not isinstance(lengths, list) and not isinstance(lengths, torch.Tensor):
            try:
                ...
                raise ValueError(
                    "lengths must be a list, torch.Tensor, or an iterable convertible to a list."
                )
        ...
        self.max_sequence_len_overall = (
            max_sequence_len_overall
            if max_sequence_len_overall is not None
            else max_tokens_effective_budget
        )

        self.long_indices_group = []
        self.short_indices_group = []
        self.skipped_indices_too_long = (
            []
        )  # Longer than even the model/budget can handle
        ...
        for i in range(len(self.lengths)):
            ...
            if l > self.max_sequence_len_overall:
                ...
                print(
                    f"Warning: Sample {i} with length {l} exceeds max_sequence_len_overall "
            ...
            if l > self.long_sample_threshold:
                if (
                    l > self.max_tokens_effective_budget
                ):  # Check if it can even fit as a batch of 1
                    # This sample is longer than threshold AND too long for budget as BS=1
                    print(
                        f"Warning: Long Sample {i} (length {l}) also exceeds max_tokens_effective_budget "
        ...
        if self.skipped_indices_too_long:
            print(
                f"Total skipped indices due to excessive length: {len(self.skipped_indices_too_long)}"
            )
    ...
    def _get_target_batch_size(self, max_allowed_bs: int) -> int:
        ...
        if (
            target_bs < self.min_batch_size and target_bs != 0
        ):  # if target_bs became 0, it means it can't fit
        ...
        if self.constrain_to_power_of_two:
            ...
            while True:
                ...
                if pow2_val >= self.min_batch_size:
                    ...
                    pow2_val == 1 and self.min_batch_size == 1 and max_allowed_bs >= 1
                ):  # Special case for min_batch_size = 1
                    current_best_pow2 = 1

                if (
                    pow2_val >= max_allowed_bs
                ):  # Optimization: no need to check higher powers if already > max_allowed_bs
                    if (
                        (1 << (p + 1)) > max_allowed_bs
                        and current_best_pow2 == 0
                        and pow2_val <= max_allowed_bs
                        and pow2_val >= self.min_batch_size
                    ):
            ...
            target_bs = max_allowed_bs
            if (
                target_bs % 2 != 0 and self.min_batch_size > 1
            ):  # If min_batch_size is 1, odd is fine
                target_bs -= 1
            if (
                target_bs < self.min_batch_size
            ):  # After making it even, it might fall below min_batch_size
                return 0
    ...
    def __iter__(self):
        ...
        for long_idx in long_indices_iter:
            if self.enable_sanity_checks and long_idx in yielded_indices_this_epoch:
                raise RuntimeError(
                    f"Sanity Check FAILED (long): Index {long_idx} already yielded."
                )
            yield [long_idx]
        ...
        short_indices_sorted = sorted(
            self.short_indices_group, key=lambda i: self.lengths[i], reverse=True
        )
        ...
        carry_over = (
            deque()
        )  # For samples that couldn't form a batch in a previous attempt
        ...
        while pool or carry_over:
            ...
            if carry_over:
                ...
                items_to_consider_for_batch_start.extend(
                    sorted(
                        list(carry_over), key=lambda i: self.lengths[i], reverse=True
                    )
                )
            ...
            if self.enable_sanity_checks and first_idx in yielded_indices_this_epoch:
                ...
                print(
                    f"Warning/Debug: Sample {first_idx} (from short/carry_over) already in yielded_indices_this_epoch. Skipping."
                )
                continue
            ...
            if (
                L_max_for_this_batch == 0
            ):  # Avoid division by zero for zero-length (empty) samples
                max_allowed_bs_for_L_max = float("inf")  # Effectively, many can fit
            else:
                max_allowed_bs_for_L_max = (
                    self.max_tokens_effective_budget // L_max_for_this_batch
                )

            if (
                max_allowed_bs_for_L_max == 0
            ):  # This first_idx itself is too long for the budget
                print(
                    f"Debug: first_idx {first_idx} (len {L_max_for_this_batch}) too long for budget "
                    f"{self.max_tokens_effective_budget}. Effective max_bs=0. Adding to carry_over."
                )
                if (
                    first_idx not in yielded_indices_this_epoch
                ):  # only carry over if not already processed
                    carry_over.append(
                        first_idx
                    )  # Try later, maybe alone if min_bs allows
                continue
            ...
            while len(current_batch_indices) < target_bs and carry_over:
                ...
                if (
                    ...
                    and candidate_idx in yielded_indices_this_epoch
                ):
                    continue
                if (
                    self.lengths[candidate_idx] <= L_max_for_this_batch
                ):  # Important check!
                ...
                if self.enable_sanity_checks:
                    for idx_cb in current_batch_indices:
                        if idx_cb in yielded_indices_this_epoch:
                            raise RuntimeError(
                                f"Sanity Check FAILED (short batch formation): Index {idx_cb} already yielded."
                            )
                ...
                if final_bs_for_partial > 0 and final_bs_for_partial <= actual_len:
                    ...
                    if self.enable_sanity_checks:
                        for idx_cb in batch_to_yield:
                            if idx_cb in yielded_indices_this_epoch:
                                raise RuntimeError(
                                    f"Sanity Check FAILED (partial short batch): Index {idx_cb} already yielded."
                                )
            ...
            if self.min_batch_size == 1:
                ...
                if self.lengths[final_idx] <= self.max_tokens_effective_budget:
                    if self.enable_sanity_checks:
                        if final_idx in yielded_indices_this_epoch:
                            raise RuntimeError(
                                f"Sanity Check FAILED (final carry): Index {final_idx} already yielded."
                            )
                        yielded_indices_this_epoch.add(final_idx)
                    yield [final_idx]
                else:
                    print(
                        f"Info: Final carry_over sample {final_idx} (len {self.lengths[final_idx]}) "
        ...
        if self.enable_sanity_checks:
            total_initial_processable = len(self.long_indices_group) + len(
                self.short_indices_group
            )
            if len(yielded_indices_this_epoch) != total_initial_processable:
                print(
                    f"Sanity Check INFO: Not all processable indices were yielded. "
    ...
    def __len__(self):
        ...
        if self.short_indices_group:
            avg_len_short = sum(
                self.lengths[i] for i in self.short_indices_group
            ) / len(self.short_indices_group)
            if avg_len_short > 0:
                ...
                # This is very rough.
                effective_min_bs = max(
                    self.min_batch_size,
                    (
                        2
                        if self.constrain_to_power_of_two and self.min_batch_size <= 1
                        else 1
                    ),
                )

                num_batches += math.ceil(
                    len(self.short_indices_group) / effective_min_bs
                )
