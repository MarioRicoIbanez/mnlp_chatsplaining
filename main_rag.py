#!/usr/bin/env python3
"""
PDF RAG system main script for processing and retrieving information from PDF books.
Features:
- PDF text extraction
- Smart chunking with max size of 256 tokens
...
def create_rag_instance(use_custom_embedding: bool = False, max_chunk_size: int = 256) -> PDFRAG:
    """Create a PDFRAG instance with the specified configuration.
...
def process_pdfs(rag_instance: PDFRAG, max_workers: int = None, batch_size: int = 1000) -> bool:
    """Process PDF files using the RAG instance.
...
def run_rebuild_mode(chunk_size: int = 256):
    """Run the system in rebuild mode to rebuild embeddings from scratch."""
...
def run_training_mode(
    ...
    """Run the system in training mode to train custom embeddings.
...
def run_custom_mode(chunk_size: int = 256, max_workers: int = None, batch_size: int = 1000):
    """Run the system using custom-trained embeddings."""
    logger.info("Custom Embedding Mode")
    logger.info("=" * 50)
...
def run_normal_mode(chunk_size: int = 256, max_workers: int = None, batch_size: int = 1000):
    """Run the system in normal processing mode."""
    logger.info("Normal Processing Mode")
    logger.info("=" * 50)
...
def run_streamlit_mode(use_custom: bool = False, chunk_size: int = 256, max_workers: int = None, batch_size: int = 1000):
    """Run the system in Streamlit chat mode.
    ...
    if use_custom:
        ...
        if not custom_model_path.exists():
            ...
            logger.info("Train a custom model first with: python main_rag.py --mode train")
            return
        logger.info("Using custom embeddings for Streamlit interface")
    else:
        logger.info("Using standard embeddings for Streamlit interface")
...
def create_argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description="PDF RAG System - Process and query PDF documents using embeddings",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                           # Normal mode with standard embeddings
  %(prog)s --mode rebuild            # Rebuild vector store from scratch
  %(prog)s --mode train              # Train custom embeddings
  ...
  %(prog)s --mode custom             # Use custom embeddings
  %(prog)s --mode streamlit          # Start Streamlit chat interface
  %(prog)s --mode streamlit --custom # Streamlit with custom embeddings
...
Note: For pushing to Hugging Face Hub, add your token to .env file:
  ...
  or login via: huggingface-hub login
    ...
    parser.add_argument(
        "--mode",
        choices=["normal", "rebuild", "train", "custom", "streamlit"],
        default="normal",
