...
class OpenCodeProcessor(BaseOpenQAProcessor):
    ...
    def get_question(self, ds_name: str, split: str, index: int) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to retrieve question for {ds_name}/{split}/{index}: {e}"
            )
            return None
    ...
    def process_dataset(self) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        try:
            ...
            dataset = load_dataset(
                "nvidia/OpenCodeReasoning-2", split="python", streaming=True
            )
        ...
        train_data = []
        target_valid_examples = (
            self.max_examples if self.max_examples is not None else float("inf")
        )
        last_log_percentage = 0
        total_checked = 0

        logger.info(
            f"Starting to process dataset, looking for {target_valid_examples} valid examples..."
        )
        ...
        for item in dataset:
            ...
            if self.max_examples is not None:
                ...
                if (
                    current_percentage >= last_log_percentage + 10
                    and current_percentage <= 100
                ):
                    logger.info(
                        f"Found {current_percentage}% of target valid examples ({len(train_data)}/{self.max_examples} valid found, checked {total_checked} total)"
                    )
            ...
            if total_checked <= 3:
                logger.info(
                    f"Item {total_checked} structure: {list(item.keys()) if isinstance(item, dict) else type(item)}"
                )
                if isinstance(item, dict):
                    logger.info(
                        f"Item {total_checked} difficulty: {item.get('difficulty')}, pass_rate: {item.get('pass_rate')}"
                    )

            # Skip if item is not a dictionary
            if not isinstance(item, dict):
                logger.warning(
                    f"Item {total_checked} is not a dictionary: {type(item)}"
                )
                continue
            ...
            if not self._is_valid_difficulty(difficulty):
                if total_checked <= 10:  # Debug first 10 items
                    logger.info(
                        f"Skipping item {total_checked} due to difficulty: {difficulty}"
                    )
                continue
            ...
            if pass_rate < 0.8:
                if total_checked <= 10:  # Debug first 10 items
                    logger.info(
                        f"Skipping item {total_checked} due to pass_rate: {pass_rate}"
                    )
                continue
        ...
        logger.info(
            f"Finished processing. Got {len(train_data)} training examples after checking {total_checked} total examples"
        )
        return (
            train_data,
            [],
            [],
        )  # Return empty lists for val/test since we only have one split
    ...
    def _process_item(self, item: Dict) -> Dict:
        ...
        try:
            ...
            if not all([ds_name, ds_split, ds_index is not None]):
                logger.warning(
                    f"Missing dataset info: dataset={ds_name}, split={ds_split}, index={ds_index}"
                )
                return None
            ...
            if question is None:
                logger.warning(
                    f"Could not retrieve question for {ds_name}/{ds_split}/{ds_index}"
                )
                return None
