...
class SciQProcessor(BaseDatasetProcessor):
    ...
    def process_dataset(self) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        logger.info(
            f"Finished processing. Got {len(train_format)} training, {len(val_format)} validation, and {len(test_format)} test examples"
        )
    ...
    def push_to_hub(
        self, repo_name: str = "RikoteMaster/sciq_treated_epfl_mcqa", token: str = None
    ):
        """
