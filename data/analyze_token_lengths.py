...
def get_token_lengths_batch(tokenizer, batch: Dict[str, List], device: str) -> List[Dict[str, int]]:
    """Calculate token lengths for a batch of examples using GPU."""
    # Tokenize in batches
    with torch.no_grad():
        question_tokens = tokenizer(batch["question"], return_tensors="pt", padding=True, truncation=True).to(device)
        answer_tokens = tokenizer(batch["answer"], return_tensors="pt", padding=True, truncation=True).to(device)
        explanation_tokens = tokenizer(batch["explanation"], return_tensors="pt", padding=True, truncation=True).to(device)
        
        # Combine texts for total length
        combined_texts = [f"{q} {a} {e}" for q, a, e in zip(batch["question"], batch["answer"], batch["explanation"])]
        total_tokens = tokenizer(combined_texts, return_tensors="pt", padding=True, truncation=True).to(device)
    
    # Calculate lengths (excluding padding)
    lengths = []
    for i in range(len(batch["question"])):
        lengths.append({
            "question_tokens": (question_tokens.attention_mask[i] == 1).sum().item(),
            "answer_tokens": (answer_tokens.attention_mask[i] == 1).sum().item(),
            "explanation_tokens": (explanation_tokens.attention_mask[i] == 1).sum().item(),
...
def main():
    parser = argparse.ArgumentParser(description="Analyze token lengths in OpenQA_merged dataset")
    parser.add_argument("--num_examples", type=int, help="Optional: Number of examples to analyze (default: full dataset)")
    ...
    if args.num_examples:
        ...
        if estimated_memory > available_memory * 0.8:  # if we need more than 80% of available memory
            logger.warning("⚠️ Warning: This might use a lot of memory. Consider using --num_examples to limit the analysis.")
    ...
    for field in ["question_tokens", "answer_tokens", "explanation_tokens", "total_tokens"]:
